...
public class SmartScriptParser {
	 ...
	 * @param tekstZaProvjeru string koji se zeli parsirati
	 * @throws SmartScriptParserException javlja gresku ako parsiranje nije uspjelo
	 */
	
	
	public SmartScriptParser(String tekstZaProvjeru) throws SmartScriptParserException {
	 ...
	 */
	public void parsiranje() throws SmartScriptParserException {
		this.stog= new ObjectStack();
		int trenutnoStanje=pocetnoStanje;
		char[] ulaz=this.documentBody.toCharArray();
		...
		try {
			for(int i=0; i< ulaz.length;i++) {
				...
				case pocetakTaga:
					...
					else if(Character.toUpperCase(ulaz[i]) != 'F' && Character.toUpperCase(ulaz[i])!='E' && ulaz[i] != '=') {
						throw new SmartScriptParserException("Kriva naredba!");
					}
					else if(Character.toUpperCase(ulaz[i])=='F' || Character.toUpperCase(ulaz[i])=='E') {
						gradiTekst.append(ulaz[i]);
				...
				case citajFor:
					...
					else if (Character.isLetter(ulaz[i]) || Character.isDigit(ulaz[i]) || ulaz[i]==' ' || ulaz[i]=='.' || (ulaz[i]=='_' && ulaz[i-1] !=' ')) {
						gradiTekst.append(ulaz[i]);
					}
					break;
				case krajTaga:
				...
				case citajFunkciju:
					...
					else if(Character.isDigit(ulaz[i]) && ulaz[i-1]=='@' || ulaz[i]=='_' && ulaz[i-1] == '@') {
	...
	private void dodajToken() throws SmartScriptParserException {
		...
		if( Character.isLetter(gradiTekst.toString().charAt(0))) {
			...
			echoNodeTokeni.add(new TokenString(gradiTekst.toString().substring(1,gradiTekst.toString().length()-1)));
		}
		else if(gradiTekst.toString().charAt(0)=='+' || gradiTekst.toString().charAt(0)=='-' || gradiTekst.toString().charAt(0)=='*' || gradiTekst.toString().charAt(0)=='/') {
	 ...
	 * @throws Ako na stogu nema nista javlja gresku
	 */
	 ...
	 * @throws Ako su neispravni parametri javlja gresku
	 */
