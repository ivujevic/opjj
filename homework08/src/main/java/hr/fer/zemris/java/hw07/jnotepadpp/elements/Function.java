 ...
 * 
 */
package hr.fer.zemris.java.hw07.jnotepadpp.elements;

import hr.fer.zemris.java.hw07.jnotepadpp.JnotepadPP;
...
public class Function {
	 ...
	 */
	public Map<String, Action> mapaAkcija = new HashMap<>();
	...
	public Function(JnotepadPP window) {
		...
		Action action = new AbstractAction("New", new ImageIcon(imgLocation)) {
		...
		action.putValue(Action.SHORT_DESCRIPTION, "Create new File for editing.");
		mapaAkcija.put("New", action);

		// Akcija za naredbu Open
		imgLocation = "images/Open24.gif";
		action = new AbstractAction("Open", new ImageIcon(imgLocation)) {
				...
				if (returnVal == JFileChooser.APPROVE_OPTION) {
					...
					try {
						...
						JOptionPane.showMessageDialog(Function.this.tabbedPane.getParent(),
								"Pogreška u čitanju datoteke", "Pogreška", JOptionPane.ERROR_MESSAGE);
					}
				}
			}
		};
		...
		action.putValue(Action.SHORT_DESCRIPTION, "Open file.");
		...
		action = new AbstractAction("Save", new ImageIcon(imgLocation)) {
		...
		action = new AbstractAction("Save As", new ImageIcon(imgLocation)) {
				...
				if (returnVal == JFileChooser.APPROVE_OPTION) {
					...
					if (file.exists()) {
						repyly = JOptionPane.showConfirmDialog(Function.this.tabbedPane.getParent(),
								"Jeste li sigurni da želite zamijeniti datoteku?", "Datoteka postoji!",
		...
		action.putValue(Action.SHORT_DESCRIPTION, "Save file with changed name.");

		mapaAkcija.put("SaveAs", action);

		// Akcija za naredbu Cut
		imgLocation = "images/Cut24.gif";
		action = new AbstractAction("Cut", new ImageIcon(imgLocation)) {
		...
		action = new AbstractAction("Copy", new ImageIcon(imgLocation)) {
		...
		action = new AbstractAction("Paste", new ImageIcon(imgLocation)) {
		...
		action.putValue(Action.SHORT_DESCRIPTION, "Paste selected text.");
		...
		action = new AbstractAction("Exit", new ImageIcon(imgLocation)) {
		...
		action = new AbstractAction("Statistical", new ImageIcon(imgLocation)) {
			...
			public void actionPerformed(ActionEvent e) {
				...
				JOptionPane.showMessageDialog(tabbedPane.getParent(), "Your document has "
						+ numberOfCharacters + " characters, " + numberOfNonBlankCharacter
						+ " non-blank characters and " + numberOfLines + " lines.", "Statistical",
		...
		action.putValue(Action.SHORT_DESCRIPTION, "Statistical information about document.");
