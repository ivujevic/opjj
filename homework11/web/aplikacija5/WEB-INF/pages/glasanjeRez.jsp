...
<body  bgcolor =${color}>
		...
		<table border="1" cellspacing = "0" class ="rez">
			...
			<tbody>
				...
				<c:forEach var = "result" items="${voteResults}">
					<tr><td><c:out value="${result.title}"/></td><td><c:out value="${result.votesCount }"/></td></tr>
					<c:if test="${result.votesCount > max}">
								<c:set var="max" value="${result.votesCount}" />
						</c:if>
				</c:forEach>
			</tbody>
	...
	<ul>
		<c:forEach var="result" items="${voteResults}">
			<c:if test="${result.votesCount == max}">
				<li><a href="${result.optionLink}" target="_blank"><c:out value = "${result.title}"/></a>
			</c:if>
		</c:forEach>
	</ul>
</body>
